        <nav class="navbar-default navbar-static-side" role="navigation"> 
            <div class="sidebar-collapse">
                <ul class="nav" id="side-menu">
                    <li class="sidebar-search">
                        <div class="input-group custom-search-form">
                            <!-- <input type="text" class="form-control" placeholder="Search...">
                            <span class="input-group-btn">
                                <button class="btn btn-default" type="button" style="height: 34px;">
                                    <i class="fa fa-search"></i>
                                </button>
                            </span> -->
                            <br></br>
                        </div>
                        <!-- /input-group -->
                    </li>
                    <li>
                        <a href="toDashboard"><i class="fa fa-dashboard fa-fw"></i> Inicio</a>
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i> Clientes<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroCliente">Registrar Cliente</a>
                            </li>
                            <li>
                                <a href="toConsultarClientes">Consultar Clientes</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i> Vehiculos<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroVehiculo">Registrar Vehiculo</a>
                            </li>
                            <li>
                                <a href="toConsultarVehiculo">Consultar Vehiculos</a>
                            </li>
                            <li>
                                <a href="toDetalleVehiculo">Detalle Vehiculo</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                                        <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i>Revisiones<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroRevision">Registrar Revision</a>
                            </li>
                            <li>
                                <a href="toConsultarRevision">Consultar Revisiones</a>
                            </li>
                            <li>
                                <a href="toDetalleRevision">Detalle Revision</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i> Marcas de Vehiculos<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroMarca">Registrar Marca</a>
                            </li>
                            <li>
                                <a href="toRegistroModelo">Registrar Modelos</a>
                            </li>
                            <li>
                                <a href="#">Consultar Marcas</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i>Fallas<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroFalla">Registrar Falla</a>
                            </li>
                            <li>
                                <a href="#">Consultar Fallas</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i>Repuestos<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroRepuesto">Registrar Repuesto</a>
                            </li>
                            <li>
                                <a href="toConsultarRepuesto">Consultar Respuestos</a>
                            </li>
                            <li>
                                <a href="toRegistroTipoRepuesto">Registrar Tipo de Repuesto</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i>Empleados<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroTrabajador">Registrar Empleado</a>
                            </li>
                            <li>
                                <a href="login.html">Consultar Empleados</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-wrench fa-fw"></i>Tipo de Automóvil<span class="fa arrow"></span></a>
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="toRegistroTipoVehiculo">Registrar Tipo de Vehiculo</a>
                            </li>
                            <li>
                                <a href="toConsultarTipoVehiculo">Consultar Tipo de Vehiculo</a>
                            </li>
                        </ul>
                        <!-- /.nav-second-level -->
                    </li>
                    
                </ul>
                <!-- /#side-menu -->
            </div>
            <!-- /.sidebar-collapse -->
        </nav>
        <!-- /.navbar-static-side -->
